...
const PHASES = [
  ...
  { v: 'v2.6', title: 'Overlays graphe réseau & Import flux', status: 'done', label: '✓ Livré', eta: '',
    items: [
      ...
      'Tooltips hover sur flux, routes et nœuds VRF ; statut réel du flux (Déployé/Validé/En attente/Refusé)',
      'Animation dashes temps-réel pour flux et routes actifs ; arrowhead bézier (suit la tangente du chemin)',
      'Autocomplete sur les 5 champs filtres flux (suggestions issues des données chargées)',
      'Bouton « Voir sur le graphe » depuis Nouveau flux : affiche le flux analysé en overlay animé',
      'Fix affichage Scripts dans le détail flux (rule_id + description par équipement)',
      ...
      'Toggle Physique / Logique / Aucune dans le panneau gauche avec animation de fondu',
      'Drag de zone : clic sur le fond d\'un conteneur déplace tous les nœuds du groupe',
      'Sauvegarde automatique des positions des nœuds en localStorage',
      ...
      'Liste zones unifiée en Administration : Zone (logique/physique) + PhysicalZone avec badges de type et filtre texte',
      'Import flux JSON : modal 4 étapes (Saisie → Aperçu → Validation → Import)',
      'Constructeur visuel drag & drop 3 panneaux (Palette / Constructeur SOURCE·DEST·RÈGLE / Aperçu JSON)',
      'Validation 3 niveaux : syntaxique (bloquant) → règles métier (avertissements) → détection doublons',
  ...
  { v: 'v2.8', title: 'Sauvegarde & Restauration', status: 'done', label: '✓ Livré', eta: '',
    items: [
      ...
      'Domaines séparés : Métier (flux, zones, équipements), Audits (ACL, routing, events), Simulation (VRF)',
  ...
  { v: 'v2.8.4', title: 'Réorg navigation & VRF config', status: 'done', label: '✓ Livré', eta: '',
    items: [
      '"Équipes & Sites" renommé en "Équipes" ; "Conformité" déplacée en section Référentiel',
      'Configuration : onglet "Zones logiques" épuré (sans sélecteur de type, sans zones physiques)',
      'Configuration : nouvel onglet "Zones physiques" dédié (DC / Salle / Baie) avec CRUD complet',
      'Configuration : nouvel onglet "VRF" — création, édition, suppression, affectation d\'équipements',
      'Topologie : onglets "Zones logiques" + "Zones physiques" (vue cartes avec type, localisation, description)',
      'Topologie zones : badge "Confiance X%" sur une seule ligne ; trait résiduel VRF supprimé',
    ] },
  ...
  { v: 'v2.10', title: 'Tests de non régression', status: 'planned', label: '🗓 Planifié', eta: '',
    items: ['Suite pytest (backend) : endpoints API, validation flux, calcul de chemin, règles ACL', 'Suite Playwright (frontend) : parcours utilisateur clés (nouveau flux, validation, topologie)', 'Rapport de couverture et comparaison avant/après chaque release', 'CI/CD : exécution automatique sur chaque push (GitHub Actions)', 'Données de test dédiées : fixtures isolées sans impact sur la base de démo'] },
